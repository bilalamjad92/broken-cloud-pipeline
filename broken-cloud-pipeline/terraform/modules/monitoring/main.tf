...
resource "aws_cloudwatch_metric_alarm" "app_health" {
  alarm_name          = "${var.name}-app-health"
  ...
  alarm_actions       = [aws_sns_topic.this.arn]
  dimensions = { HealthCheckId = var.app_health_check }
}
resource "aws_cloudwatch_metric_alarm" "jenkins_health" {
  alarm_name          = "${var.name}-jenkins-health"
  metric_name         = "HealthCheckStatus"
  namespace           = "AWS/Route53"
  statistic           = "Average"
  period              = 60
  ...
  comparison_operator = "LessThanThreshold"
  alarm_actions       = [aws_sns_topic.this.arn]
  dimensions = { HealthCheckId = var.jenkins_health_check }
}
